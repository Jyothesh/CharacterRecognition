...
/**
 * This is the base class with Kohonen Neural Network
 */
import com.txstate.actual.gui.OCR;
import com.txstate.actual.som.training.TrainingSet;

public class KNetwork extends Network {
	...
	//Error rate above this variable
	...
	public KNetwork(int inputCount, int outputCount, OCR owner) {
		totalError = 1.0 ;
	...
	//Copy weights from one to another network
	public static void copyWeights(KNetwork dest, KNetwork source) {
	...
	//Feed an input pattern to network
	...
	//Input pattern and get the winner
	...
	//This is the place where majority of the learning process is done 
	//and evaluates the weights against the training set
	...
	//Adjust the weights based on the previous training iteration
	...
	//This one is called for training the network
	public void learn() throws RuntimeException {
		...
		KNetwork bestnet;  // Preserve best here
		totalError = 1.0 ;
		...
		bestnet = new KNetwork(inputNeuronCount, outputNeuronCount, ocr);
		...
		for(iter=0; ; iter++) {
			...
			ocr.updateStats(n_retry,totalError,best_err);
			if(halt) {
				ocr.updateStats(n_retry, totalError, best_err);
				break;
			}
